...
def test(model, dataset, visualizer, opt, bg_info, test_steps=0, gen_vid=False, lpips=True):
    ...
    for i in range(0, total_num, opt.test_num_step): # 1 if test_steps == 10000 else opt.test_num_step
        ...
        if "ray_mask" in model.output and "ray_masked_coarse_raycolor" in opt.test_color_loss_items:
            ...
            filepath = os.path.join("/home/xschen/yjcai/vis/test_{}/images".format(38), filename)
            ...
            filepath = os.path.join(
                 "/home/xschen/yjcai/vis/test_{}/images".format(38), filename)
            ...
            filename = 'step-{:04d}-{}-vali.npy'.format(i, "ray_mask")
            filepath = os.path.join(
                 "/home/xschen/yjcai/vis/test_{}/images".format(38), filename)
            np.save(filepath, ray_masks.view(height, width).cpu().numpy())
            ...
            if i%10==0:
                test_psnr_sum += mse2psnr(loss).item()
            else:
                train_psnr_sum += mse2psnr(loss).item()
    ...
    psnr = visualizer.get_psnr(opt.test_color_loss_items[-1])
    # visualizer.reset()
    ...
    if gen_vid:
        ...
        print('--------------------------------Finish generating vid--------------------------------')
    print (test_psnr_sum/3, train_psnr_sum/27, '-=============')
    return psnr, test_psnr_sum/3, train_psnr_sum/27
    ...
    for epoch in range(epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(data_loader):
            ...
            if total_steps == 10000 or (total_steps % opt.test_freq == 0 and total_steps < (opt.maximum_step - 1) and total_steps > 0):
                ...
                model.opt.no_loss = 1
                with torch.no_grad():
                    test_psnr, nntest_psnr, nntrain_psnr = test(model, test_dataset, Visualizer(test_opt), test_opt, test_bg_info, test_steps=total_steps, lpips=True)
                model.opt.no_loss = 0
                model.opt.is_train = 1
                best_iter = total_steps if test_psnr > best_PSNR else best_iter
                best_PSNR = max(nntest_psnr, best_PSNR)
                visualizer.print_details(f"test at iter {total_steps}, PSNR: {nntest_psnr}, best_PSNR: {best_PSNR}, best_iter: {best_iter} {test_psnr} {nntrain_psnr}")
            model.train()
