...
def test(total_steps, model, dataset, visualizer, opt, bg_info, test_steps=0, gen_vid=False, lpips=True, max_test_psnr=0, \
    ...
    for i in range(total_num): # 1 if test_steps == 10000 else opt.test_num_step
        ...
        tmpgts = {}
        tmpgts["segres_mask"] = data['segres'].clone()
        tmpgts["gt_image"] = data['gt_image'].clone()
        tmpgts["gt_image_1over8"] = data['gt_image_1over8'].clone()
        ...
        range_min.append(output["range_min"][0])
        range_max.append(output["range_max"][0])
        if opt.proposal_nerf:
            nerf_coarse_raycolor = output['nerf_coarse_raycolor']
        curr_visuals = model.get_current_visuals(data=data)
        ori_pred = curr_visuals['final_coarse_raycolor']
        ori_gt = tmpgts['gt_image']

        if opt.mask_moving_obj:
            pred = ori_pred.clone().reshape(height, width, 3).cuda()
            gt = ori_gt.clone().reshape(height, width, 3).cuda()
            segres_mask = tmpgts['segres_mask'].squeeze()[..., None].cuda()
            pred = pred.mul(segres_mask)
            gt = gt.mul(segres_mask)
        ...
        preds.append(np.asarray(ori_pred.detach().squeeze().cpu().reshape(height, width, 3)))
        gts.append(np.asarray(ori_gt.detach().squeeze().cpu().reshape(height, width,3)))
        ...
        if seq_frame_index>=sequence_length_list[seq_index]:
            ...
            ssim_train_half, ssim_test_half = [],[]
            lpips_train_half_vgg, lpips_test_half_vgg = [],[]
    #range_min = torch.stack(range_min)
    #range_max = torch.stack(range_max)
    #print (torch.min(range_min, -2))
    #print (torch.max(range_max, -2))
    #exit()
...
def init_distributed_mode(args, verbose=True):
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        os.environ['MASTER_PORT'] = str(getattr(args, 'port', '26666'))
...
#    opt.local_rank = os.environ['LOCAL_RANK']
    ...
    for epoch in range(epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        #### test model
        if epoch % opt.test_freq == 0 or epoch==1 or epoch==0:
        ...
        if (opt.nerf_create_points and (epoch % opt.iter_pg == 0) and epoch>=400 and epoch<=4000) or epoch==0:
            model.opt.is_train = 0
            model.opt.no_loss = 1
            model.eval()

            #if opt.probe_hole_type=='pointnerf':
#            if True:
#                prob_opt = copy.deepcopy(test_opt)
#                prob_opt.name = opt.name
#                add_xyz, add_embedding, add_conf = probe_hole(model, train_dataset, Visualizer(prob_opt), prob_opt, None, test_steps=total_steps, opacity_thresh=opt.prob_thresh)
            #elif opt.probe_hole_type=='nerf':
