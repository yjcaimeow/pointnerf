...
def test(total_steps, model, dataset, visualizer, opt, bg_info, test_steps=0, gen_vid=False, lpips=True, max_test_psnr=0, max_train_psnr=0):
    print('-----------------------------------Testing-----------------------------------')
    model.eval()
    total_num = dataset.total
    ...
    for epoch in range(epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(data_loader):
            ...
            if total_steps == 1000 or (total_steps % opt.test_freq == 0 and total_steps < (opt.maximum_step - 1) and total_steps > 0):
                ...
                model.opt.no_loss = 1
                with torch.no_grad():
                    test_psnr, train_psnr = test(total_steps, model, train_dataset, Visualizer(test_opt), test_opt, test_bg_info, test_steps=total_steps, lpips=True)
                model.opt.no_loss = 0
                model.opt.is_train = 1
                #del test_dataset
