...
class GeneratorBlock(nn.Module):
    ...
    def forward(self, x, prev_rgb, istyle, inoise=None):
        if exists(self.upsample):
            x = self.upsample(x)
        if inoise!=None:
...
class Generator(nn.Module):
    def __init__(self, image_size, latent_dim, network_capacity = 16, transparent = False, attn_layers = [], no_const = False, fmap_max = 512, init_channels=128):
        super().__init__()
        self.image_size = image_size
        self.latent_dim = latent_dim
        self.num_layers = int(log2(image_size) - 6)
...
#        self.to_rgb = RGBBlock(latent_dim, 64, False, False)

    def forward(self, styles, initial=None, input_noise=None, guide=None, guide_512=None):
