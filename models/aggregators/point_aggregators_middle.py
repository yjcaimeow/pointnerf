...
class PointAggregator_middle(torch.nn.Module):

    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        parser.add_argument(
            '--feature_init_method_m',
            type=str,
            default="rand",
        ...
        parser.add_argument(
            '--shading_feature_mlp_layer0_m',
            type=int,
            default=0,
            help='interp to agged features mlp num')

        parser.add_argument(
            '--shading_feature_mlp_layer1_m',
            type=int,
            default=2,
            help='interp to agged features mlp num')

        parser.add_argument(
            '--shading_feature_mlp_layer2_m',
            type=int,
            default=0,
            help='interp to agged features mlp num')

        parser.add_argument(
            '--shading_feature_mlp_layer3_m',
            type=int,
            default=2,
            help='interp to agged features mlp num')

        parser.add_argument(
            '--shading_feature_num_m',
            type=int,
            default=256,
        ...
        parser.add_argument(
            '--shading_alpha_mlp_layer_m',
            type=int,
            default=1,
            help='agged features to alpha mlp num')

        parser.add_argument(
            '--shading_color_mlp_layer_m',
            type=int,
            default=1,
            help='agged features to alpha mlp num')

        parser.add_argument(
            '--shading_color_channel_num_middle',
            type=int,
            default=128,
        ...
        parser.add_argument(
            '--agg_alpha_xyz_mode_m',
            type=str,
            default="None",
                help='which agg xyz mode to use [None not to use | world world xyz | pers perspective xyz ]')

        parser.add_argument(
            '--agg_color_xyz_mode_m',
            type=str,
            default="None",
                help='which agg xyz mode to use [None not to use | world world xyz | pers perspective xyz ]')
    ...
    def __init__(self, opt):
        ...
        super(PointAggregator_middle, self).__init__()
    ...
    def viewmlp(self, sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, sample_loc_w, sample_ray_dirs, vsize, weight, pnt_mask_flat, pts, viewdirs, local_viewdirs, total_len, ray_valid, in_shape, dists):
        ...
        local_viewdirs = local_viewdirs[ray_valid, :]
        ...
        output_placeholder = torch.zeros([total_len, self.opt.shading_color_channel_num_middle + 1], dtype=torch.float32, device=output.device)
    ...
    def forward(self, sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, sample_loc_w, sample_ray_dirs, sample_local_ray_dirs, vsize, grid_vox_sz):
        ...
        if total_len == 0 or torch.sum(ray_valid) == 0:
            ...
            return torch.zeros(in_shape[:-1] + (self.opt.shading_color_channel_num_middle + 1,), device=ray_valid.device, dtype=torch.float32), ray_valid.view(in_shape[:-1]), None, None
        ...
        return output.view(in_shape[:-1] + (self.opt.shading_color_channel_num_middle + 1,)), ray_valid.view(in_shape[:-1]), weight, conf_coefficient
