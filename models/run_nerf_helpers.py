...
class MultiHeadAttentionMulti(nn.Module):
    def __init__(self, args, output_ch=3, height=160, width=240):
        """
        """
        super(MultiHeadAttentionMulti, self).__init__()
        self.K = args.close_points_number
        self.skips = [4]
        ...
        ### extra multi res
        self.fn_thetaV_m = nn.Linear(args.bottleneck_size+27, args.net_ch_m)
        self.fn_thetaK_m = nn.Linear(args.bottleneck_size+27, args.net_ch_m)
        self.fn_thetaQ_m = nn.Linear(27, args.net_ch_m)
        self.multi_head_attention_m = Attention(args=args, dim=args.net_ch_m)

        self.fn_thetaV_l = nn.Linear(args.bottleneck_size+27, args.net_ch_l)
        self.fn_thetaK_l = nn.Linear(args.bottleneck_size+27, args.net_ch_l)
        self.fn_thetaQ_l = nn.Linear(27, args.net_ch_l)
        self.multi_head_attention_l = Attention(args=args, dim=args.net_ch_l)

        if self.render_type == 'stylegenerator':
            from neural_render.stylegan2_pytorch_multi import StyleVectorizer, Generator
            self.S = StyleVectorizer(emb=args.z_dim, depth=8)
            self.G = Generator(image_size=512, latent_dim=args.z_dim, network_capacity=args.network_capacity)
        elif self.render_type == 'condition_generator':
            from neural_render.condition_generator import CondConvGenerator
            self.neural_render_2d = CondConvGenerator()

    def latent_to_w(iself, style_vectorizer, latent_descr):
        return [(style_vectorizer(latent_descr.cuda()), 8)]
        #return [(style_vectorizer(z.cuda()), num_layers) for z, num_lpyers in latent_descr]

    def styles_def_to_tensor(self, styles_def):
        return torch.cat([t[:, None, :].expand(-1, n, -1) for t, n in styles_def], dim=1)

    def forward(self,rays, points, directions, point_cloud=None, height=160, width=240, bs=1, z=None, batch_img_fea=None, \
               points_256=None, rays_256=None, points_512=None, rays_512=None):
        """
        rays    : BHW  , 27
        ...
        #### res 256
        ppd_k_256 = self.fn_thetaK_m(points_256) # pre_processed_k
        ppd_v_256 = self.fn_thetaV_m(points_256) # pre_processed_v
        ppd_q_256 = self.fn_thetaQ_m(rays_256)   # pre_processed_q
        rgb_feature_map_256 = self.multi_head_attention_m(ppd_q_256, ppd_k_256, ppd_v_256)
        # rgb_feature_map_256 = self.down_channel(rgb_feature_map_256)
        #### res 256
        ppd_k_512 = self.fn_thetaK_l(points_512) # pre_processed_k
        ppd_v_512 = self.fn_thetaV_l(points_512) # pre_processed_v
        ppd_q_512 = self.fn_thetaQ_l(rays_512)   # pre_processed_q
        rgb_feature_map_512 = self.multi_head_attention_l(ppd_q_512, ppd_k_512, ppd_v_512)
        ...
        if self.render_type=='cnn':
            return self.neural_render_2d(input_pts.reshape(-1, height, width, self.input_ch), batch_img_fea=batch_img_fea)
        elif self.render_type == 'stylegenerator':
            w_space = self.latent_to_w(self.S, z)
            w_styles = self.styles_def_to_tensor(w_space)
            return self.G(w_styles, initial=input_pts.reshape(-1, height, width, self.input_ch).permute(0,3,1,2), \
                point_guid_256=rgb_feature_map_256, point_guid_512=rgb_feature_map_512)
            # return self.G(w_styles, initial=input_pts.reshape(-1, height, width, self.input_ch).permute(0,3,1,2))
        elif self.render_type == 'condition_generator':
            return self.neural_render_2d(input_pts.reshape(-1, height, width, self.input_ch).permute(0,3,1,2), z=z)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,rays, points, directions, point_cloud=None, height=160, width=240, bs=1, z=None, batch_img_fea=None, \
        ...
        rgb_feature_map = self.multi_head_attention(ppd_q,ppd_k,ppd_v)
        #### res 256
        #ppd_k_256 = self.fn_thetaK(points_256) # pre_processed_k
        #ppd_v_256 = self.fn_thetaV(points_256) # pre_processed_v
        #ppd_q_256 = self.fn_thetaQ(rays_256)   # pre_processed_q
        #rgb_feature_map_256 = self.multi_head_attention(ppd_q_256, ppd_k_256, ppd_v_256)
        #rgb_feature_map_256 = self.down_channel(rgb_feature_map_256)
        ...
        if self.render_type=='cnn':
            ...
            w_styles = self.styles_def_to_tensor(w_space)
            return self.G(w_styles, initial=input_pts.reshape(-1, height, width, self.input_ch).permute(0,3,1,2))
            #return self.G(w_styles, initial=input_pts.reshape(-1, height, width, self.input_ch).permute(0,3,1,2), point_guid_256=rgb_feature_map_256)
        elif self.render_type=='mlp':
            h = input_pts
            ...
            return self.neural_render_2d(input_pts.reshape(-1, height, width, self.input_ch).permute(0,3,1,2))
            #return self.neural_render_2d(input_pts.reshape(-1, height, width, self.input_ch).permute(0,3,1,2), z=z)
class Rasterization(nn.Module):
    def __init__(self, args, output_ch=3, height=128, width=192):
        """
        """
        super(Rasterization, self).__init__()
        self.skips = [4]
        self.embed_dir = args.embed_dir
        self.render_type = args.render_type
        if self.embed_dir:
    ...
    def forward(self, embedded_dirs, batch_depth, batch_imgfea, viewdirs, height=128, width=192, bs=1):
        if self.embed_dir:
            input_pts = torch.cat((batch_depth.unsqueeze(1), batch_imgfea, embedded_dirs), dim=1)
        else:
            input_pts = torch.cat((batch_depth.unsqueeze(1), batch_imgfea, viewdirs), dim=1)
        if self.render_type=='cnn':
...
class ProposalNeRF(nn.Module):
    def __init__(self, D=4, W=128, input_ch=60, input_ch_views=24, output_ch=4, skips=[4], use_viewdirs=True):
        """
        """
        super(ProposalNeRF, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
...
#            self.weight_linear = nn.Linear(W, 1)
    ...
    def forward(self, input_pts, input_views=None):
        ...
        if self.use_viewdirs:
            alpha = self.density_super_act(self.alpha_linear(h))
...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=60, input_ch_views=24, output_ch=4, skips=[4], use_viewdirs=True):
        """
        """
        ...
        if use_viewdirs:
            ...
            self.density_super_act = torch.nn.Softplus()
            self.weight_linear = nn.Linear(W, 1)
            #self.rgb_linear = nn.Linear(W//2, W//2)
        else:
            self.output_linear = nn.Linear(W, output_ch)
    ...
    def forward(self, input_pts, input_views):
        ...
        if self.use_viewdirs:
            alpha = self.density_super_act(self.alpha_linear(h))
            feature = self.feature_linear(h)
            weight = torch.sigmoid(self.weight_linear(h))
            return feature, alpha, weight
            #h = torch.cat([feature, input_views], -1)
