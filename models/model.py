...
class Decoder(nn.Module):
    def __init__(self, model_opt=None, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
        # decoder mapping network
        self.feature_encoder_in_channels = 128
        self.size = 512
        self.style_dim = 256 * 2
        thumb_im_size = 128
        self.lr_mapping = 0.01
        layers = [PixelNorm(),
                   EqualLinear(
                       self.style_dim // 2, self.style_dim, lr_mul=self.lr_mapping, activation="fused_lrelu"
                   )]

        for i in range(4):
            layers.append(
                EqualLinear(
                    self.style_dim, self.style_dim, lr_mul=self.lr_mapping, activation="fused_lrelu"
                )
            )
        ...
        for i in range(self.log_in_size+1, self.log_size + 1):
            ...
            self.convs.append(
                StyledConv(in_channel, out_channel, 3, self.style_dim, upsample=True,
                           blur_kernel=blur_kernel, project_noise=self.project_noise)
            )
...
class Generator(nn.Module):
    def __init__(self, model_opt=None, renderer_opt=None, blur_kernel=[1, 3, 3, 1], ema=False, full_pipeline=True):
        super().__init__()
        self.size = 512
        self.style_dim = 256
        self.num_layers = 1
        self.train_renderer = True
        self.feature_encoder_in_channels = 128
    ...
    def forward(self, features, styles, return_latents=False,
