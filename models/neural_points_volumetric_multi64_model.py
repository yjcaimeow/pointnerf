...
class NeuralPointsVolumetricMulti64Model(BaseRenderingModel):
...
class NeuralPointsRayMarching(nn.Module):
    def __init__(self,
        ...
        if self.opt.neural_render=='cnn':
            ...
            self.G = Generator(image_size=512, latent_dim=self.opt.z_dim, network_capacity=self.opt.network_capacity, init_channels=self.opt.shading_color_channel_num, use_guide=self.opt.multi_res, \
                               combination=self.opt.combination)
    ...
    def forward(self,
        ...
        sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, \
            ...
            self.neural_points({"pixel_idx": pixel_idx, "camrotc2w": camrotc2w, "campos": campos, "near": near, "far": far,"focal": focal, "c2w":c2w, \
                                "h": h, "w": w, "intrinsic": intrinsic,"gt_image":gt_image, "raydir":raydir, 'vsize':self.opt.vsize, "local_raydir":local_raydir})
        ...
        if self.opt.neural_render=='cnn':
            ...
            guide=None
            if self.opt.multi_res:
                guide = output['coarse_raycolor_m'].reshape(1, img_h*2, img_w*2, self.opt.shading_color_channel_num_middle).permute(0,3,1,2)
            output['final_coarse_raycolor'] = self.G(w_styles, initial=output['coarse_raycolor'].reshape(1, img_h, img_w, self.opt.shading_color_channel_num).permute(0,3,1,2), guide=guide).reshape(1, -1, 3)
        return output
