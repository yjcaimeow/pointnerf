...
prob_tiers=" 10000 30000 60000 100000 150000 400000"
...
#TOOLS="srun --partition=$PART --gres=gpu:${GPUNUM} -n$NODENUM --ntasks-per-node=1 -x SH-IDC1-10-142-5-[65,90,99,139,188]"
TOOLS="srun --partition=$PART --gres=gpu:${GPUNUM} -n$NODENUM --ntasks-per-node=1 -w SH-IDC1-10-142-5-188"
$TOOLS --job-name=$JOBNAME sh -c "CUDA_VISIBLE_DEVICES=$3 python train_ft.py \
