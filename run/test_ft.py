...
def test(model, dataset, visualizer, opt, bg_info, test_steps=0, gen_vid=True, lpips=True):
    ...
    for i in range(0, total_num, opt.test_num_step): # 1 if test_steps == 10000 else opt.test_num_step
        ...
        #np.savez('/mnt/lustre/caiyingjie/data/scannet/scans/scene0101_00_with_insert/results_pointnerf_'+str(opt.scan)+'_'+str(vid)+'.npz', sample_loc=visuals['sample_loc'], sample_loc_w=visuals['sample_loc_w'], ray_valid=visuals["ray_valid"], \
        #         decoded_features=visuals['decoded_features'])
