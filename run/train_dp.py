...
from utils.util import add_flour, init_distributed_mode, setup_for_distributed
import gc
        ...
        if len([n for n in glob.glob(opt.checkpoints_dir + opt.name + "/*_net_ray_marching.pth") if os.path.isfile(n)]) > 0:
            ...
            if load_points == 2:
                ...
                for scan_idx, scan in enumerate(opt.scans):
                    ...
                    if opt.embed_init_type=='random' or 1==1:
                        ...
                        points_conf_all = torch.randn([1, len(points_xyz_all)], device=points_xyz_all.device, dtype=torch.float32)
                        print('|||||||||||||||', points_embedding_all.shape, points_color_all.shape)
                    else:
