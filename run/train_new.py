...
def test(t_models, model, data_loader, visualizer, opt, bg_info, test_steps=0, gen_vid=False, lpips=False, writer=None, epoch=None, height=480, width=640, dirname=None):
    ...
    for i, data in enumerate(data_loader):
        ...
        if opt.progressive_distill and opt.pseudo_gt_load_type=='offline':
        ...
        for k in range(0, totalpixel, chunk_size):
            ...
            if opt.progressive_distill:
                if opt.pseudo_gt_load_type=='offline':
                    ...
                    #if k==0:
                    #    dstname = '/mnt/cache/caiyingjie/code/pointnerf_new/'+'epoch_'+str(epoch)+'_vid'+str(vid)+'_debug_npz.npz'
                    #    np.savez(dstname, decoded_features=data['decoded_features_loaded'].cpu().numpy(), sample_loc=data['sample_loc_loaded'].cpu().numpy())
                    #    print('======= save done ========', dstname)
                else:
                    with torch.no_grad():
                        ...
                        #if k==0:
                        #    dstname = '/mnt/cache/caiyingjie/code/pointnerf_new/'+'epoch_'+str(epoch)+'_vid'+str(vid)+'_debug_2.npz'
                        #    np.savez(dstname, decoded_features=curr_visuals['decoded_features'].cpu().numpy(), sample_loc=curr_visuals['sample_loc'].cpu().numpy())
                        #    print('======= save done ========', dstname)
...
def main():
            ...
            if load_points == 2:
                ...
                for scan_idx, scan in enumerate(opt.scans):
                    ...
                    if opt.embed_init_type=='random':
                        ...
                        points_conf_all = torch.randn([1, len(points_xyz_all), 1], device=points_xyz_all.device, dtype=torch.float32)
                    else:
    ...
    for epoch in range(epoch_count, opt.maximum_epoch+1):
        ...
        for i, data in enumerate(data_loader):
            ...
            if opt.pseudo_gt_load_type == 'online':
                with torch.no_grad():
                    seq_id = data["seq_id"].item()
                    t_models[seq_id].eval()
                    t_models[seq_id].set_input(data)
                    ...
                    curr_visuals = t_models[seq_id].get_current_visuals(data=data)
                    #for key, value in curr_visuals.items():
                    #    if key!='coarse_raycolor':
                    #        print(key, value.shape)
                    #loss = torch.nn.MSELoss().to("cuda")(curr_visuals['gt_image'], curr_visuals['coarse_raycolor'])
                    #psnr = mse2psnr(loss)
                    #cprint.warn("psnr is {}".format(psnr.item()))
                    #print (data['sample_loc_loaded'].shape, curr_visuals['sample_loc'].shape, (curr_visuals['sample_loc']==data['sample_loc_loaded']).all())
                    #print (data['decoded_features_loaded'].shape, curr_visuals['decoded_features'].shape, (curr_visuals['decoded_features']==data['decoded_features_loaded']).all())
                    #cprint.err(data['decoded_features_loaded'])
                    #cprint.warn(curr_visuals['decoded_features'])
                    #exit()
                    data['sample_loc_loaded'] = curr_visuals['sample_loc']
                    data['sample_loc_w_loaded'] = curr_visuals['sample_loc_w']
                    data['ray_valid_loaded'] = curr_visuals['ray_valid']
